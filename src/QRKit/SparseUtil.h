...
* \class TripletArray
*
* \brief A structure to hold a std::vector of triplets
* It allows for more readable sparse code than using push_back directly.
*
* \sa SparseMatrix::setFromTriplets()
*/

template<typename Scalar, typename _StorageIndex = typename SparseMatrix<Scalar>::StorageIndex >
class TripletArray : public std::vector<Triplet<Scalar, _StorageIndex>>
{
public:
  typedef Triplet<Scalar, _StorageIndex> triplet_t;
  typedef std::vector<triplet_t> Base;

  typedef _StorageIndex Index;

  /* Construct, reserving size elements. */
  TripletArray(size_t size = 0) {
    reserve(size);
  }

  /* Add triplet to the array */
  void add(Index i, Index j, Scalar v) {
    push_back(triplet_t(i, j, v));
  }

  /* Add triplet to the array if v != 0 */
  void add_if_nonzero(Index i, Index j, Scalar v) {
    if (v != Scalar(0)) {
      add(i, j, v);
    }
  }
